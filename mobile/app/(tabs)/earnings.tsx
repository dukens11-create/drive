...
export default function EarningsScreen() {
  const { metrics } = useDriveRealtime();
  ...
  return (
    <View className="flex-1 bg-zinc-50 p-4 dark:bg-zinc-950">
      <View className="rounded-3xl bg-white p-5 shadow-soft dark:bg-zinc-900">
        ...
        <Text className="mt-3 text-sm text-zinc-600 dark:text-zinc-300">{metrics.tripsCompleted} completed trips across {metrics.hoursOnline.toFixed(1)} online hours.</Text>
      </View>
    </View>
  );
}

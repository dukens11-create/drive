...
export default function InboxScreen() {
  ...
  const sendTripMessage = async () => {
    if (!activeTrip || !chatMessage.trim()) {
      return;
    }
    setIsSubmitting(true);
    try {
      await ridesApi.message(activeTrip.rideId, chatMessage.trim());
      setChatMessage('');
      await refreshData();
    } catch (sendError) {
          ...
          <View className="mt-2 gap-3">
            {activeTrip ? (
              <View className="rounded-2xl bg-white p-4 shadow-soft dark:bg-zinc-900">
                <Text className="text-sm font-semibold text-zinc-900 dark:text-zinc-100">Trip chat · {activeTrip.riderName}</Text>

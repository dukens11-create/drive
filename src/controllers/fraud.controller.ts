import * as service from '../services/fraud.service';
...
export async function listAlerts(req: any, res: any) {
  const result = await service.listFraudAlerts({ ...req.query, ...req.body, actor: req.user });
...
export async function chargeback(req: any, res: any) {
  const result = await service.reportChargeback({ ...req.body, actor: req.user });
  if (result.error) return res.status(400).json(result);
  res.json(result);
}

export async function chargebacks(req: any, res: any) {
  const result = await service.listChargebacks({ ...req.query, actor: req.user });
  res.json(result);
}

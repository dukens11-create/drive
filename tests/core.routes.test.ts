...
test('GET / serves the professional dashboard login page', async () => {
  ...
  test('GET /driver-dashboard.html is compatible with the default CSP', async () => {
    await withServer(async baseUrl => {
      ...
      ['toggle-availability-button', 'Driver mode', 'Professional control center', 'Ride History', 'Real-time Map', 'Performance Stats', 'Support \/ Help', 'Driver dashboard navigation'].forEach(label => {

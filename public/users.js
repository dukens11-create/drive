...
async function submitAuth(path, body, button) {
  ...
  try {
    ...
    if (path.endsWith('/login') && normalizeRole(payload.user?.role) === 'driver') {
      throw new Error('Please sign in through the driver portal');
    }

    persistSession(payload);
    showMessage('success', 'Authentication successful. Redirecting...');
    setTimeout(() => {
      window.location.href = getRedirectPath(payload.user?.role || RIDER_ROLE);
    }, REDIRECT_DELAY_MS);
  } catch (error) {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  try {
    return Boolean(JSON.parse(user || '{}')?.id);
  } catch {
    return false;
  }
})()) {
    ...
    let role = RIDER_ROLE;
    try {
      role = JSON.parse(user || '{}').role || RIDER_ROLE;
    } catch {
      role = RIDER_ROLE;
    }
  ...
  signupForm.addEventListener('submit', async event => {
    ...
    await submitAuth('/api/auth/signup', { email, password, role: RIDER_ROLE }, signupButton);
  });
});
